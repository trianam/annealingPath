...
class Smoothener:
    ...
    def plot(self, V, plotter, plotStartEnd, plotInnerVertexes, plotEdges, plotSpline):
        k = 4
        den = float(len(V) - k + 1)
        p = []
        for i in range(k):
        ...
        p = np.array(p)
        if plotEdges:
            plotter.plot(V[:,0], V[:,1], 'r--')
        if plotStartEnd:
            plotter.plot(V[0,0], V[0,1], 'ro')
            plotter.plot(V[-1,0], V[-1,1], 'ro')
        if plotInnerVertexes:
            plotter.plot(V[1:-1,0], V[1:-1,1], 'ro')
        if plotSpline:
    ...
    def _bSpline(self, V, k, part, step = 0.001):
        """
