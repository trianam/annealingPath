...
configuration of the path, or in other words is the ordered vector
$\mathbf{v}=(\mathbf{v}_1,\dots,\mathbf{v}_n)$ of all $n$ vertexes $\mathbf{v}_i=(x_i,y_i)$ of the
path. The problem~\eqref{eq:opt} is formulated in two different
versions in the project. The first one is:
\begin{equation*}
\begin{aligned}
...
The second version is similar, but the function to minimize is
...
The simulated annealing aim is to find the minimum saddle points in
the curve represented by the equation~\eqref{eq:lagrangianFunProj}. 
